...
const OrderList = observer(({ onSelect }: EntityListScreenProps) => {
  ...
  const [executeDeleteMutation] = useMutation(DELETE_ORDER);
  ...
  const openEditor = useCallback(
    (id?: string) => {
      const params: OpenInBreadcrumbParams = {
        breadcrumbCaption: intl.formatMessage({id: 'screen.OrderDetails'}),
...
function getUpdateFn(e: any) {
  return (cache: ApolloCache<any>) => {
    cache.modify({
      fields: {
        listOrders(existingRefs, { readField }) {
