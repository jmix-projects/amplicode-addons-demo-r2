...
const DELETE_SALESPERSON = gql`
    mutation deleteSalesperson($id: Long!) {
        deleteSalesperson(id: $id)
    }
`;

const SalespersonList = observer(({onSelect}: EntityListScreenProps) => {
    ...
    const [executeDeleteMutation] = useMutation(DELETE_SALESPERSON);
    ...
    const openEditor = useCallback(
        (id?: string) => {
            const params: OpenInBreadcrumbParams = {
                breadcrumbCaption: intl.formatMessage({id: 'screen.SalespersonDetails'}),
    ...
    const items = data?.["listSalespeople"];
    ...
    return (
        <div className="narrow-layout">
            ...
            <GeoMap centerX={50.2004} centerY={53.2261} zoom={11}>
                ...
                <VectorLayer id='salespersonsLayer' items={items} propertyName={'location'} clustering={true}/>
            </GeoMap>
        </div>
    )
        ;
...
const Fields = ({entity}: { entity: any }) => (
    <>
        {Object.keys(entity)
            .filter(p => p !== "id" && p !== "location" && entity[p] != null)
            .map(p => (
                <div key={p}>
...
function getUpdateFn(e: any) {
    return (cache: ApolloCache<any>) => {
        cache.modify({
            fields: {
                listSalespeople(existingRefs, {readField}) {
